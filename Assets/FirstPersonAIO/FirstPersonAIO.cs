...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private const float jumpRayLength = 0.3f;
  ...
  public class BETA_SETTINGS{
    ...
    private void FixedUpdate() {
        ...
        // if (Input.GetButton("Sprint") || Input.GetAxis("Sprint") >= 0.5)
        // {
        //     isSprinting = true;
        // }
        // else
        ...
        if (useStamina) {
            if (backgroundStamina > 0)
                isSprinting = false;
            if (isSprinting == true && backgroundStamina > 0) { backgroundStamina -= staminaDepletionSpeed; } else if (backgroundStamina < (Stamina * 10) && !Input.GetButton("Sprint")) { backgroundStamina += staminaDepletionSpeed / 2; }
        }
        ...
        // With moving the pivot to a sensible point, this breaks
        Ray ray = new Ray(transform.position + Vector3.up, -transform.up);
        if (IsGrounded || fps_Rigidbody.velocity.y < 0.1) {
            // Ray now originates from 1m above pivot. jumpRayLength describes coyote height
            RaycastHit[] hits = Physics.RaycastAll(ray, 1 + jumpRayLength);
        ...
        if (IsGrounded && didJump && jumpPower > 0 && playerCanMove) {
            yv += jumpPower;
            IsGrounded = false;
            didJump = false;
        }
        ...
        if (useHeadbob == true || dynamicFootstep.useDynamicFootStepProcess == true) {
            ...
            xPos = 0;
            zTilt = 0;
            if (useJumdLandMovement) { xTilt = -springPosition * landAngle; } else { xTilt = -springPosition; }
        ...
        if (dynamicFootstep.useDynamicFootStepProcess) {
            ...
            // TODO: Untangle this fucking garbage
            if (Physics.Raycast(transform.position + Vector3.up, dwn, out hit)) {
                ...
                if (IsGrounded) {
                    if (!previousGrounded) {
                        // Dynamic footsteps landing with phys mat hit
                ...
                // Play default sounds, no phys mat found
